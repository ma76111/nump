...
bot.on('message', async (msg) => {
  ...
  if (msg.photo) {
    try {
      ...
      // التحقق من عدم تجاوز 20 صورة
      if (userProofPhotos[userId].length >= 20) {
        return bot.sendMessage(userId, '⚠️ لقد وصلت للحد الأقصى (20 صورة)');
      }
...
async function handleNewTask(userId, isAdmin) {
  ...
  if (cancelledTask) {
    ...
    if (elapsedTime <= 2 * 60 * 1000) {
      ...
      // حساب الوقت المتبقي
      const timeQuery = db.prepare("SELECT (strftime('%s', 'now') - strftime('%s', created_at)) as elapsed_seconds FROM tasks WHERE id = ?").get(task.id);
  ...
  if (activeTask) {
    ...
    // حساب الوقت المتبقي
    const timeQuery = db.prepare("SELECT (strftime('%s', 'now') - strftime('%s', created_at)) as elapsed_seconds FROM tasks WHERE id = ?").get(activeTask.id);
    const elapsedMinutes = Math.floor(timeQuery.elapsed_seconds / 60);
    const remainingMinutes = 90 - elapsedMinutes;
...
function handleHowToWork(userId) {
  ...
  if (!videoFileId || videoFileId === 'none') {
    return bot.sendMessage(userId, 
      `📖 طريقة العمل:\n\n` +
      `لم يتم رفع فيديو توضيحي بعد.\n` +
      `يرجى التواصل مع الدعم للمساعدة.`
    );
  }
  
  bot.sendVideo(userId, videoFileId, {
    caption: '📖 شرح طريقة العمل\n\nشاهد الفيديو لمعرفة كيفية إتمام المهام بشكل صحيح'
  }).catch((error) => {
    logError('Error sending how to work video', error, userId);
    bot.sendMessage(userId, '❌ حدث خطأ في إرسال الفيديو. يرجى المحاولة لاحقاً.');
  });
}
